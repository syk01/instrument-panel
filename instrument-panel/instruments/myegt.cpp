...
MyEGT::MyEGT(int xPos, int yPos, int size) : instrument(xPos, yPos, size)
{
    setName("MyEGT");
    addVars();
...
void MyEGT::resize()
{
    ...
    ALLEGRO_BITMAP* orig = loadBitmap("my-egt.png");
    addBitmap(orig);
...
void MyEGT::update()
{
    ...
    // Calculate values EGT
    egtAngle = 62 - (simVars->exhaustGasTemp - 502) * 0.15;
    if (egtAngle < -60) {
        egtAngle = -60;
    }
    else if (egtAngle > 62) {
    ...
    // Calculate values fuelPressure
    flowAngle = 118 + simVars->fuelPressure * 6;
    if (flowAngle < 118) {
        flowAngle = 118;
    }
...
void MyEGT::addVars()
{
    globals.simVars->addVar(name, "ENG EXHAUST GAS TEMPERATURE:1", false, 1, 0);
    globals.simVars->addVar(name, "GENERAL ENG FUEL PRESSURE:1", false, 1, 0);
}
